...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		if (!StringUtils.hasLength(beanName) || !this.targetSourcedBeans.contains(beanName)) {
			// advisedBeans 用于存储不可代理的 bean，如果包含直接返回
	 ...
	 *
	 * 这里便是整个 AOP 的入口，内部的逻辑简述就是，如果被代理的目标类实现了一个或多个自定义接口，
	 * 那么就会使用 JDK 动态代理，如果没有实现任何接口，会使用 CGLIB 实现代理，如果设置了 proxy-target-class="true"，
	 * 那么就都会使用 CGLIB
	 *
	 * @see #getAdvicesAndAdvisorsForBean
	 */
	...
	protected Object getCacheKey(Class<?> beanClass, @Nullable String beanName) {
		if (StringUtils.hasLength(beanName)) {
			...
			return (FactoryBean.class.isAssignableFrom(beanClass) ?
					// 如果是工厂 bean，那么则在 beanName 前面加上工厂 bean 的前缀，如果不是
					// 工厂 bean，那么直接返回 beanName
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// 可以看到如果没有匹配到 specificInterceptors，那么 specificInterceptors == null，
		// 那么便会进入 else，则不会生成代理类
	 ...
	 *
	 * 这个方法主要是在内部创建了一个 ProxyFactory 的实例，然后 set 了一大堆内容，
	...
	protected Object createProxy(
		...
		if (!proxyFactory.isProxyTargetClass()) {
			// 这里便是判断 proxy-target-class 的值是 true 还是 false
			// 如果为 true 的话，那么便用 cglib 来生成代理类
