...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
			@Nullable Constructor<?>[] chosenCtors,
										   // explicitArgs 参数，是我们调用 #getBean(...)
										   // 方法时传递进来的。一般该参数，该参数就是用于初始化
										   // Bean 时所传递的参数。如果该参数不为空，
										   // 则可以确定构造函数的参数就是它了。
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			// 对构造函数进行排序处理
			// public 构造函数优先参数数量降序，非 public 构造函数餐厨数量降序
			...
			for (Constructor<?> candidate : candidates) {
				...
				// 如果已经找到选用的 构造函数或者需要的参数个数小雨当前的构造函数参数个数，则终止
				// 因为，已经按照参数个数降序排列了
				...
				if (resolvedValues != null) {
					try {
						...
						// 若发生 UnsatisfiedDependencyException 异常，添加到 cause 中
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (explicitArgs != null) {
			...
			// 缓存中存在则解析存储在 BeanDefinition 中的参数
			// 如给定方法的构造函数 A(int, int), 则通过此方法后就会把配置文件中的 ("1", "1") 转换成 (1, 1)
			// 缓存中的值可能是原始值也有可能是最终值
			if (argsToResolve != null) {
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						try {
							...
							// ParameterNameDiscoverer 适用于解析方法和构造函数的参数名称的接口，为参数名称探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							// 获取指定构造函数的参数名称
							if (pnd != null) {
								paramNames = pnd.getParameterNames(candidate);
					...
					// 严格模式：解析构造函数时，比如所有的都需要匹配，否则抛出异常
					// 宽松模式：使用具有 "最接近的模式" 进行匹配
					// typeDiffWeight: 类型差异权重
					int typeDiffWeight = (mbd.isLenientConstructorResolution() ?
							argsHolder.getTypeDifferenceWeight(paramTypes) : argsHolder.getAssignabilityWeight(paramTypes));
					...
					// 但是仅在非宽松构造函数解析模式下执行该检查，并显式忽略重写方法（具有相同的参数签名）
