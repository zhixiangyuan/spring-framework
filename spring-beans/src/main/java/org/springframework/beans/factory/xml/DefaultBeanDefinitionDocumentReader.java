...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 记录老的 BeanDefinitionParseDelegate 对象
		BeanDefinitionParserDelegate parent = this.delegate;
		// 检查 <beans>...</beans> 根标签的命名空间是否为空，
		// 或者是 http://www.springframework.org/schema/beans
		...
		// 设置 delegate 回老的 BeanDefinitionParerDelegate
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		try {
			// ResourcePatternUtils.isUrl(location) 中判断以 classpath*: 或 classpath: 开头的为绝对路径
		...
		if (absoluteLocation) {
			...
			try {
				// 添加配置文件地址的 Resource dao actualResources 中，并加载相应的 BeanDefinition
		...
		// 相对路径
			...
			try {
				...
				// 不存在
					...
					// 添加配置文件地址的 Resource 到 actualResource 中，并加载相应的 BeanDefinition
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		// 进行 bean 元素解析
		// 如果解析成功，则返回返回 BeanDefinitionHolder 对象，而 BeanDefinitionHolder
		// 为 name 和 alias 的 BeanDefinition 对象
		// 如果解析失败，则返回 null
