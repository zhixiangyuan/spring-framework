...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 调用所有实现了 PriorityOrdered 的 BeanDefinitionRegistryPostProcessors
		...
		for (String ppName : postProcessorNames) {
			// 已经处理过了的，跳过
			if (processedBeans.contains(ppName)) {
				// skip - already 6processed in first phase above
	...
	public static void registerBeanPostProcessors(
		...
		// 获取所有的 BeanPostProcessor 的 beanName，但是这些 bean 还没有实例化
		...
		// 注册 BeanPostProcessorChecker 同样是一个 BeanPostProcessor，它主要是用于在 BeanPostProcessor 实例化期间记录日志
